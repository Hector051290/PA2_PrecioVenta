...
# Zonas exactas usadas en el Colab (después del replace)
ZONAS = ["Cono Este", "Cono Norte", "Cono Oeste", "Cono Sur"]

# Columnas que espera el modelo (resultado de get_dummies con drop_first=True sobre X_train)
# X tenía: Propiedad, m2, Habitaciones, Banos, Ofertas, Zona
# get_dummies(drop_first=True) sobre Zona genera: Zona_Cono Norte, Zona_Cono Oeste, Zona_Cono Sur
# (Cono Este queda como referencia y se elimina)
COLUMNAS_MODELO = [
    'Propiedad', 'm2', 'Habitaciones', 'Banos', 'Ofertas',
    'Zona_Cono Norte', 'Zona_Cono Oeste', 'Zona_Cono Sur'
]
...
def load_model(filename: str):
    ...
    if response.status_code != 200:
        st.error(f"❌ No se pudo descargar '{filename}' (HTTP {response.status_code}). "
                 f"Verifica que el archivo exista en: {url}")
        st.stop()
    return joblib.load(BytesIO(response.content))

def preparar_features(m2, habitaciones, banos, zona, propiedad=1, ofertas=1):
    """
    Replica exactamente el mismo preprocesamiento del Colab:
    pd.get_dummies con drop_first=True sobre la columna Zona,
    alineado con las columnas que vio X_train.
    """
    fila = pd.DataFrame([{
        'Propiedad':    propiedad,
        'm2':           m2,
    ...
    # Alinear con las columnas exactas que el modelo conoce
